...
load_modules() {
	...
	//
	snprintf(module_path, sizeof(module_path),
		BACKSLASHDIR("%s/mod/encoder-x264", "%smod\\encoder-x264"),//encoder-video
		ga_root);
...
//#endif
	...
	//FILE * pFile = fopen ("salidaHookCommon.txt","w");
	//fprintf (pFile, "Texto de salida del hook Common");
	//fclose (pFile);
...
//#endif
	int s = 0, err;
	int kbitrate[] = { 1000, 4000, 8000 };//{ 2000, 8000 };
	...
	ga_log ("##### Reconfigure thread started ...\n");
...
//#endif
		...
		if(m_vencoder->ioctl) {
			...
			if(err < 0) {
				...
				//fprintf (pFile, "reconfigure encoder failed, err = %d.\n", err);
			} else {
				...
				//fprintf (pFile, "reconfigure encoder OK, bitrate=%d; bufsize=%d; framerate=%d/%d.\n",
				//		reconf.bitrateKbps, reconf.bufsize,
				//		reconf.framerate_n, reconf.framerate_d);
			}
		}
