...
load_modules() {
	...
	if((m_filter = ga_load_module("mod/filter-rgb2yuv", "filter_RGB2YUV_")) == NULL)
		return -1;
	if((m_vencoder = ga_load_module("mod/encoder-x264", "vencoder_")) == NULL)//"mod/encoder-video", "mod/encoder-x264"
		return -1;
	...
	if((m_ctrl = ga_load_module("mod/ctrl-sdl", "sdlmsg_replay_")) == NULL)
		return -1;
	if((m_server = ga_load_module("mod/server-live555", "live_")) == NULL)//"mod/server-live555", "mod/server-ffmpeg"
		return -1;
	return 0;
}
...
test_reconfig(void *) {
	int s = 0, err;
	int kbitrate[] = { 8000, 8000 };//{ 2000, 8000 };
	int framerate[][2] = { { 10, 1 }, {60, 1}, {10, 1} };//{ { 12, 1 }, {30, 1}, {24, 1} };
...
#endif
		reconf.framerate_n = framerate[s%2][0];
		reconf.framerate_d = framerate[s%2][1];
		// vsource
		/*if(m_vsource->ioctl) {
			err = m_vsource->ioctl(GA_IOCTL_RECONFIGURE, sizeof(reconf), &reconf);
			if(err < 0) {
				ga_error("\n\n######RECONFIGURE vsource failed, err = %d.\n\n", err);
			} else {
				ga_error("\n\n######RECONFIGURE vsource OK, framerate=%d/%d.\n\n",
						reconf.framerate_n, reconf.framerate_d);
			}
		}*/
		// encoder
		if(m_vencoder->ioctl) {
			//ESTE NO CAMBIA EL FRAMERATE
			err = m_vencoder->ioctl(GA_IOCTL_RECONFIGURE, sizeof(reconf), &reconf);
			if(err < 0) {
				ga_error("\n######RECONFIGURE encoder failed, err = %d.\n\n", err);
			} else {
				ga_error("\n######RECONFIGURE encoder OK, bitrate=%d; bufsize=%d; framerate=%d/%d.\n\n",
