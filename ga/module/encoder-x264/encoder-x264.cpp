...
vencoder_reconfigure(int iid) {
	...
	if(vencoder_reconf[iid].id >= 0) {
		...
		ga_log("***PRE***x264_encoder_parameters Ok->crf=%d; framerate=%d/%d; bitrate=%d/%d; bufsize=%d\n",
		...
		if(reconf->bufsize > 0) {
			ga_log("***Cambia bufsize\n");
			params.rc.i_vbv_buffer_size = reconf->bufsize;
			doit++;
		}

		//params.i_width = reconf->width;//TODO: MODIFICACION
		//params.i_height = reconf->height;//TODO: MODIFICACION
		//
		if(doit > 0) {
			//ga_log("######## x264_encoder_reconfig\n");
			if(x264_encoder_reconfig(encoder, &params) < 0) {
				...
				x264_param_t params2;
				x264_encoder_parameters(encoder, &params2);
				//SI QUE LLEGA AQUI
				ga_log("video encoder: reconfigured. crf=%.2f; framerate=%d/%d; bitrate=%d/%dKbps; bufsize=%dKbit.\n",
						params2.rc.f_rf_constant,
						params2.i_fps_num, params2.i_fps_den,
						params2.rc.i_bitrate, params2.rc.i_vbv_max_bitrate,
						params2.rc.i_vbv_buffer_size);
			}
			ga_log("x264_encoder_reconfig Ok\n");
		}
		reconf->id = -1;
	}
